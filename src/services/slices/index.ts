  ...
  reducers: {
    addIngredient: (state, action: PayloadAction<TIngredient>) => {
      ...
      if (!state.ingredients) {
        console.warn('⚠️ state.ingredients was undefined, initializing');
        state.ingredients = [];
      }
      ...
      console.log('🔍 New constructor state:', {
        bun: state.bun,
    ...
    removeIngredient: (state, action: PayloadAction<string>) => {
      ...
      // ✅ Проверяем, что ingredients существует
      if (!state.ingredients) {
        state.ingredients = [];
        return state;
      }
    ...
    clearConstructor: (state) => {
      console.log('🔍 clearConstructor called');
      state.bun = null;
      state.ingredients = []; // ✅ Всегда массив
      return state;
    }
  }
});
...
const initialAuthState: AuthState = {
  user: null,
  isAuth: false, // ✅ Временно установим false
  isLoading: false,
  error: null,
